...
Граф и его транспонированная версия имеют одни и те же `SCC`.
    a. Запускаем `DFS` на исходном графе, добавляем вершины в `stack` во время посещения.
    b. Обрабатываем вершины в порядке выхода из `stack`: если вершина на втором проходе не посещена, то запускаем `DFS` от нее (как от стартовой), но на транспонированном графе.
    c. Вершины, полученные на во время прохода `DFS` образуют одну из компонент связности. Продолжаем, пока `stack` не опустеет.
...
class Graph {
    int V;
    std::vector<std::vector<int>> adj;
    std::vector<std::vector<int>> adj_trans;
...
public:
    ...
    std::vector<std::vector<int>> kosaraju_alg() {
        std::stack<int> stack = DFS();              // первый проход DFS
        std::vector<std::vector<int>> strong_comp;
        std::vector<bool> visited(V, false);        // Посещение вершины на adj_trans
        
        while (!stack.empty()) {
            int v = stack.top();
            stack.pop();
            std::stack<int> temp_stack;
            DFS_util(v, temp_stack, visited, adj_trans);   // Второй проход DFS на adj_trans
