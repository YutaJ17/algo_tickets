...
#### Алгоритм Кана
1. Находим все вершины с нулевой входящей степенью
2. Помещаем их в очередь
3. Пока очередь не пуста:
    - Извлекаем вершину и добавляем ее в результат сортировки
    - Удаляем все исходящие из нее ребра 
    - Если у соседней вершины входящая степень стала нулевой - добавляем ее в очередь

#### Алгоритм на основе DFS
1. Запускаем DFS из каждой непосещенной вершины
2. После обработки всех соседей добавляем вершину в стек. 
3. Извлекаем вершины из стека - это и будет топологическим порядком.
...
class Graph {
    ...
    void topSortUtil(int v, std::vector<bool>& visited, std::stack<int> stack) {
        visited[v] = true;

        // посещаем всех соседей рекурсивно
